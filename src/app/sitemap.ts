...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  return [
    ...
    ...["about", "contact", "privacy", "blog"].map((page) => ({ url: `${config.url}/${page}`, lastModified: new Date(), changeFrequency: "monthly" as const, priority: 0.4 })),
    ...
    ...blogs.map((blog) => ({ url: `${config.url}/blog/${blog.slug}`, lastModified: new Date(blog.publishedAt), changeFrequency: "weekly" as const, priority: 0.6 })),
  ];
}
