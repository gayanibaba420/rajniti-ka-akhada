...
export default async function Home() {
  ...
  if (!lead) {
    return <div className="container-main py-20 text-center"><h1 className="text-2xl font-black">अभी कोई प्रकाशित खबर नहीं</h1></div>;
  }

  return (
    <>
      <section className="container-main py-5"><DbAdSlot position="HEADER" /></section>
      <section className="container-main grid gap-5 pb-10 lg:grid-cols-[1.65fr_.85fr]">
        <article className="story-link surface relative min-h-[420px] overflow-hidden rounded-2xl sm:min-h-[540px]">
          ...
          <div className="absolute inset-x-0 bottom-0 p-5 text-white sm:p-9">
            <div className="mb-3 flex flex-wrap gap-2"><span className="rounded bg-[#a71d2a] px-3 py-1 text-sm font-black">बड़ी खबर</span>{lead.location && <span className="flex items-center gap-1 rounded bg-white/15 px-3 py-1 text-sm backdrop-blur"><MapPin size={14}/> {lead.location}</span>}</div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
          {articles.slice(1, 3).map((article) => <StoryCard article={article} categories={categories} key={article.slug} />)}
        </div>
      </section>
      ...
      <section className="container-main py-12">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_1fr_.72fr]">{hisar[0] && <StoryCard article={hisar[0]} priority categories={categories} />}<div className="grid gap-5">{hisar.slice(1).map((a) => <StoryCard key={a.slug} article={a} horizontal categories={categories} />)}</div><div className="surface rounded-xl p-5"><h3 className="font-black">हिसार उप-श्रेणियां</h3><div className="mt-4 grid grid-cols-2 gap-2">{["शहर", "आदमपुर", "हांसी", "नारनौंद", "मंडी भाव", "क्राइम", "शिक्षा", "मौसम"].map((name) => <Link href={`/search?q=${name}`} className="rounded-lg border p-3 text-center text-sm font-bold hover:border-[var(--brand)]" style={{borderColor:"var(--line)"}} key={name}>{name}</Link>)}</div></div></div>
      </section>

      <section className="bg-[var(--surface)] py-12"><div className="container-main grid gap-8 lg:grid-cols-[1fr_330px]"><div><h2 className="section-title">हरियाणा की हलचल</h2><div className="grid gap-6 sm:grid-cols-2">{haryana.slice(0, 4).map((a) => <StoryCard key={a.slug} article={a} categories={categories} />)}</div></div><div><DbAdSlot position="SIDEBAR" /><div className="mt-6"><SidebarList items={trending.length ? trending : articles.slice().sort((a, b) => b.views - a.views).slice(0, 5)} /></div></div></div></section>
